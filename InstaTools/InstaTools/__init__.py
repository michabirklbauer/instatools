...
class Instagram:
	...
	def getBiography(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				biography = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["biography"]
			except:
				pass

		if biography != 0:
			return str(biography)
		else:
			print("It seems retrieving the biography was unsuccessful!")
			return 1

	def getFollowerCount(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				followers_count = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["edge_followed_by"]["count"]
			except:
				pass
		...
		if followers_count != -1:
			...
			print("It seems retrieving the follower count was unsuccessful!")
			return "1"

	def getFollowingCount(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				following_count = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["edge_follow"]["count"]
			except:
				pass
		...
		if following_count != -1:
			...
			print("It seems retrieving the following count was unsuccessful!")
			return "1"

	def getMediaCount(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				media_count = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["edge_owner_to_timeline_media"]["count"]
			except:
				pass
		...
		if media_count != -1:
			...
			print("It seems retrieving the media count was unsuccessful!")
			return "1"

	def getFullName(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				full_name = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["full_name"]
			except:
				pass
		...
		if full_name != 0:
			...
			print("It seems retrieving the full name was unsuccessful!")
			return 1

	def getProfilePic(self, user_name, download = False):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				profile_pic_url = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["profile_pic_url_hd"]
			except:
				pass

		if profile_pic_url != 0:
			if download:
				s_path = "" + str(profile_pic_url).split("/")[-1].split("?")[0]
				ur.urlretrieve(str(profile_pic_url), s_path)
			return str(profile_pic_url)
		else:
			print("It seems retrieving the profile pic was unsuccessful! If download was specified: Nothing was downloaded!")
			return 1

	def getNewPost(self, user_name, download = False):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
	...
	def getNewIGTV(self, user_name, download = False):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
	...
	def isProfilePrivate(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				is_private = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["is_private"]
			except:
				pass
		...
		if is_private != -1:
			...
			print("It seems retrieving the private status was unsuccessful!")
			return 1

	def isProfileBusiness(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				is_business = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["is_business_account"]
			except:
				pass
		...
		if is_business != -1:
			...
			print("It seems retrieving the business status was unsuccessful!")
			return 1

	def isProfileVerified(self, user_name):
		url = "https://www.instagram.com/" + str(user_name)
		page = ur.urlopen(url).read().decode("utf-8")
		lines = page.splitlines()
		...
		if data != "":
			jdata = json.loads(data)
			try:
				is_verified = jdata["entry_data"]["ProfilePage"][0]["graphql"]["user"]["is_verified"]
			except:
				pass
		...
		if is_verified != -1:
			...
			print("It seems retrieving the verified status was unsuccessful!")
			return 1
	...
	def getStories(self, user_name, download = False):
...
class InstaBot:
	...
	def getMediaCount(self, user_name):
		time.sleep(5)
		return self.instagram.getMediaCount(user_name)

	def getProfilePic(self, user_name):
		time.sleep(5)
		return self.instagram.getProfilePic(user_name)
	...
	def getNewPost(self, user_name):
		return self.instagram.getNewPost(user_name)

	def getStories(self, user_names):
		self.sendMessage("Today's Stories:")
		time.sleep(5)
		for user_name in user_names:
			try:
				stories = self.instagram.getStories(user_name)
			except:
	...
	def createConfig(self, user_names):

		users_media_counts = []
		users_profile_pics = []

		for user_name in user_names:
			users_media_counts.append(self.getMediaCount(user_name))
			users_profile_pics.append(self.getProfilePic(user_name))
	...
	def instaBot(self, user_names, rtime = 3480, stime = 21, daily = False, debugging_mode = True):

		if debugging_mode:
			self.sendMessage("InstaBot process was launched!")

		user_names_string = ""
		for user_name in user_names:
			user_names_string = user_names_string + str(user_name) + "; "
		...
		if rtime != 0:
			...
			while rep < rtime:
				...
				for i in range(len(user_names)):
					rep = rep + 5
					new_media_count = self.getMediaCount(user_names[i])
					if new_media_count > int(users_media_counts[i]):
						users_media_counts[i] = new_media_count
						rep = rep + 5
						new_post = self.getNewPost(user_names[i])
						...
						for item in new_post[1:]:
							...
							self.sendMessage(text)
						text = "Link to profile: https://instagram.com/"+str(user_names[i])
						self.sendMessage(text)
						self.writeConfig(users_media_counts, users_profile_pics)
					rep = rep + 5
					new_profile_pic_url = self.getProfilePic(user_names[i])
					...
					for i in range(len(user_names)):
						...
						if new_media_count > int(users_media_counts[i]):
							...
							for item in new_post[1:]:
								text = "Raw image can be found here: "+item+" ;"
								self.sendMessage(text)
							text = "Link to profile: https://instagram.com/"+str(user_namess[i])
							self.sendMessage(text)
							self.writeConfig(users_media_counts, users_profile_pics)
						new_profile_pic_url = self.getProfilePic(user_names[i])
						if new_profile_pic_url != users_profile_pics[i]:
...
class InstaToolsApp:
	...
	def __init__(self, master):
		self.master = master
		master.title("InstaTools App by Micha Birklbauer")

		# header
		mainFrame = tk.Label(master, text="\n\nInstaTools-App by Micha Birklbauer\n\n")
		...
		# desc2
		desc2Frame = tk.Label(master, text="Enter a Username:", width=100, height=3)
		...
		# get username
		get_username_button = tk.Button(master, text="Get Full Name!", width=15, command=self.get_fullname)
	...
	def get_fullname(self):
		...
		f_username = self.instagram.getFullName(f_userid)
		f_query = "Query: Get full name for username '" + str(f_userid) + "'\n"
	...
	def get_mediacount(self):
		...
		f_query = "Query: Get Media Count for username '" + str(f_userid) + "'\n"
	...
	def get_profilepic(self):
		...
		f_query = "Query: Get Profile Picture for username '" + str(f_userid) + "'\n"
	...
	def get_newpost(self):
		...
		f_query = "Query: Get newest Post for username '" + str(f_userid) + "'\n"
		f_output = ""
	...
	def get_newigtv(self):
		...
		f_query = "Query: Get newest IGTV Post for username '" + str(f_userid) + "'\n"
		f_output = ""
		for item in f_new_post:
			f_output = f_output + "Output: " + str(item) + "\n"
		self.output.config(state=tk.NORMAL)
	...
	def get_tagged(self):
		...
		f_query = "Query: Get Tagged Page for username '" + str(f_userid) + "'\n"
	...
	def get_stories(self):
		...
		f_query = "Query: Get Stories for username '" + str(f_userid) + "'\n"
		f_output = ""
	...
	def get_privacy(self):
		...
		f_query = "Query: Get Privacy Setting for username '" + str(f_userid) + "'\n"
		if f_privacy:
	...
	def dl_profilepic(self):
		...
		self.result = [f_profile_pic]
		f_query = "Query: Download Profile Picture for username '" + str(f_userid) + "'\n"
	...
	def dl_newpost(self):
		...
		self.result = f_new_post
		f_query = "Query: Download newest Post for username '" + str(f_userid) + "'\n"
		f_output = ""
		for item in f_new_post:
			f_output = f_output + "Output: " + str(item) + "\n"
	...
	def dl_newigtv(self):
		...
		self.result = f_new_post[1:]
		f_query = "Query: Download newest IGTV Post for username '" + str(f_userid) + "'\n"
		f_output = ""
		for item in f_new_post:
			f_output = f_output + "Output: " + str(item) + "\n"
		f_output = f_output + "Output: Download finished!\nOutput: File in directory: '.'\n"
	...
	def instabot(self):
		...
		while True:
			...
			confirm = input("Do you want to add another username? Yes/No\n")
			if str(confirm) in ["no", "No", "NO", "n", "0"]:
				break
		i = InstaBot("instabot.cfg", channel, api_token)
		if debugging_mode == "True":
			if daily == "True":
				i.instaBot(user_ids, int(rtime), int(stime), False, True)
			else:
				i.instaBot(user_ids, int(rtime), int(stime), True, True)
		else:
			if daily == "True":
				i.instaBot(user_ids, int(rtime), int(stime), False, False)
			else:
				i.instaBot(user_ids, int(rtime), int(stime), True, False)
	...
	def help(self):
		...
		webbrowser.open("https://github.com/t0xic-m/instatools/blob/master/README.md")

	def credits(self):
		webbrowser.open("https://raw.githubusercontent.com/t0xic-m/instatools/master/LICENSE.md")
